...
public class Matrix {
    ...
    private static final double UNKNOWN_VALUE = -1e308;

    // Data
    public ArrayList<double[]> m_data = new ArrayList<>(); //matrix elements
    ...
    /// Marshals this object into a helpers.Json DOM
    public Json marshal() {
        Json list = Json.newList();
        for (int i = 0; i < rows(); i++) {
            double[] r = m_data.get(i);
            Json list2 = Json.newList();
            for (double v : r) list2.add(v);
            list.add(list2);
        }
        return list;
    }
    ...
    public void newColumn(int numValues) {
        m_data.clear();
    ...
    public int findOrCreateValue(int column, String val) {
        ...
        if (i == null) {
            ...
            Integer integer = nextVal;
            m_enum_to_str.get(column).put(integer, val);
            m_str_to_enum.get(column).put(val, integer);
            return nextVal;
        } else
            return i;
    }
    ...
    public int valueEnum(int attr, String val) {
        ...
        if (i == null) {
            ...
            for (Map.Entry<String, Integer> stringIntegerEntry : m_str_to_enum.get(attr).entrySet()) {
                ...
                s.append("->");
                s.append(entry.getValue().intValue());
            }
    ...
    public Vec row(int index) {
        return new Vec(m_data.get(index));
    }
    ...
    private static class SortComparator implements Comparator<double[]> {
        ...
        public int compare(double[] a, double[] b) {
            if (ascending) {
                ...
                return Double.compare(b[column], a[column]);
            }
        }
    }
    ...
    private double Matrix_pythag(double a, double b) {
    ...
    private double Matrix_safeDivide(double n, double d) {
        if (d == 0.0 && n == 0.0)
            return 0.0;
        else {
    ...
    private double Matrix_takeSign(double a, double b) {
...
*/
    ...
    private SVDResult singularValueDecompositionHelper(boolean throwIfNoConverge, int maxIters) {
        int m = rows();
        int n = cols();
        if (m < n)
        ...
        // Diagonalize the bi-diagonal matrix
    ...
    /// Returns the Moore-Penrose pseudo-inverse of this matrix
...
/*
	...
	double[] firstPrincipalComponent(Random rand)
	{
		...
		for(int i = 0; i < 200; i++)
		{
			t.fill(0.0);
			for(int j = 0; j < r; j++)
				vec.Vec.addScaled(t, row(j), vec.Vec.dotProduct(row(j), p));
			double d = Math.sqrt(vec.Vec.squaredMagnitude(t));
			vec.Vec.normalize(t);
			double[] tt = p;
			p = t;
			t = tt;
	...
	void gramSchmidt(double[] v)
	{
		...
		for(int i = 0; i < r; i++)
		{
			double d = vec.Vec.dotProduct(row(i), v);
			vec.Vec.addScaled(row(i), v, -d);
		}
	}


	/// Returns a matrix containing the first k principal components of this matrix. (Destroys the contents of this matrix.)
	matrix.Matrix pca(int k, Random rand, double[] centroid)
	{
		int c = cols();
		int r = rows();
		for(int i = 0; i < c; i++)
		...
		matrix.Matrix m = new matrix.Matrix(k, c);
		for(int i = 0; i < k; i++)
		{
			double[] p = firstPrincipalComponent(rand);
			gramSchmidt(p);
			vec.Vec.copy(m.row(i), p);
		}
		return m;
	}*/
}
